...
Функция messageПолучитьСообщениеТелеграм(Запрос)
	...
	Если Константы.НовыйПользователь.Получить() = Истина Тогда
		Запись = РегистрыСведений.АккаунтыПользователей.СоздатьМенеджерЗаписи();
		Запись.ЧатИД = ЧатИД;
		Запись.Субдомен = "ГотовоКЗаписи";
		Запись.ИмяПользователя = ИмяПользователя;
		Запись.Прочитать();
		Запись.Субдомен = ПакетСообщения.message.text;
		Запись.Записать();
		Константы.НовыйПользователь.Установить(Ложь);
		ОтправитьСообщениеТГ(ЧатИД, "Успешно!");
	Иначе
		Пока Выборка.Следующий() Цикл
			Если Не Выборка.ЧатИД = ЧатИД Тогда
				Константы.НовыйПользователь.Установить(Истина);
			Иначе
				Константы.НовыйПользователь.Установить(Ложь);
		...
		Если Константы.НовыйПользователь.Получить() = Истина Тогда
			...
			Запись.ИмяПользователя = ИмяПользователя;
			Запись.Записать();
			ОтправитьСообщениеТГ(ЧатИД, "У бота нет данных о твоей организации в amoCRM" + Символы.ПС
		...
		Иначе
			...
			Если ПакетСообщения.message.text = "/start" Тогда
				ПриветственноеСообщение = "Хало, с помощью этого бота ты можешь:" + Символы.ПС + "/gif - Получить гифку"
					+ Символы.ПС + "/cur - Получить курс $" + Символы.ПС + "/wthr - Получить прогноз погоды"
