...
Функция messageПолучитьСообщениеТелеграм(Запрос)
	...
	ИначеЕсли Константы.ДобавитьЧек.Получить() = Истина Тогда
		...
		ОтправитьСообщениеТГ(ЧатИД, "Задачи успешно показаны!" + Символы.ПС + "Для продолжения работы с CRM введите /crm");
		Константы.СРМЧек.Установить(Ложь);
...
Процедура ПолучитьЗадачиНаСервере(ЧатИД)
	...
	Пока Счетчик < КоличествоЗадач Цикл
		...
		ЗадачиОтвет = ЗадачиОтвет + (Счетчик + 1) + ". " + ПакетСообщения._embedded.tasks[Счетчик].text + " до " + Формат(TimestampВДату(ПакетСообщения._embedded.tasks[Счетчик].complete_till), "ДЛФ=ДД") + Символы.ПС;
		Счетчик = Счетчик + 1;
